	...
	Citation = {
		...
		patterns: {
			...
			law: [
				...
				// "Priv. L. No. 98-23"
				[
					"(pub(?:lic)?|priv(?:ate)?)\\.? +l(?:aw)?\\.?(?: +No\\.?)? +(\\d+)[-–](\\d+)", function(match) {
					return {
						type: match[1].match(/^priv/i) ? "private" : "public",
						congress: match[2],
						number: match[3]
					}
				}],
		...
		standardize: {
			...
			law: function(data) {
				return {
					id: [data.type, "law", data.congress, data.number].join("_")
				}
			}
		},
