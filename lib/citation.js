	...
	Citation = {
		...
		find: function(text, options) {
			...
			return _.compact(_.flatten(_.map(Citation.patterns[type], function(pattern) {
				...
				var regex;
				// context means surrounding regex with generic matchers of the given size
				// beginning is non-greedy, to not gobble up start of the citation
				// ending is greedy, so that it doesn't just stop at 0
				if (context > 0)
